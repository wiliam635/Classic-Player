...
void ClassicPlayerAudioProcessor::getStateInformation(juce::MemoryBlock& destination)
{
    ...
    for (int i = 0; i < Sf2Engine::layerCount; ++i)
    {
        ...
        state.setProperty("analogAmpAttack" + juce::String(i + 1), analog.ampAttackMs, nullptr);
        state.setProperty("analogAmpDecay" + juce::String(i + 1), analog.ampDecayMs, nullptr);
        state.setProperty("analogAmpSustain" + juce::String(i + 1), analog.ampSustain, nullptr);
        state.setProperty("analogAmpRelease" + juce::String(i + 1), analog.ampReleaseMs, nullptr);
        state.setProperty("analogFilterAttack" + juce::String(i + 1), analog.filterAttackMs, nullptr);
        state.setProperty("analogFilterDecay" + juce::String(i + 1), analog.filterDecayMs, nullptr);
        state.setProperty("analogFilterSustain" + juce::String(i + 1), analog.filterSustain, nullptr);
        state.setProperty("analogFilterRelease" + juce::String(i + 1), analog.filterReleaseMs, nullptr);
        state.setProperty("analogLfoRate" + juce::String(i + 1), analog.lfoRateHz, nullptr);
    ...
    if (auto xml = getXmlFromBinary(data, size))
    {
        ...
        if (state.isValid())
        {
            ...
            for (int i = 0; i < Sf2Engine::layerCount; ++i)
            {
                ...
                analog.ampAttackMs = static_cast<float>(state.getProperty("analogAmpAttack" + analogKey, analog.ampAttackMs));
                analog.ampDecayMs = static_cast<float>(state.getProperty("analogAmpDecay" + analogKey, analog.ampDecayMs));
                analog.ampSustain = static_cast<float>(state.getProperty("analogAmpSustain" + analogKey, analog.ampSustain));
                analog.ampReleaseMs = static_cast<float>(state.getProperty("analogAmpRelease" + analogKey, analog.ampReleaseMs));
                analog.filterAttackMs = static_cast<float>(state.getProperty("analogFilterAttack" + analogKey, analog.filterAttackMs));
                analog.filterDecayMs = static_cast<float>(state.getProperty("analogFilterDecay" + analogKey, analog.filterDecayMs));
                analog.filterSustain = static_cast<float>(state.getProperty("analogFilterSustain" + analogKey, analog.filterSustain));
                analog.filterReleaseMs = static_cast<float>(state.getProperty("analogFilterRelease" + analogKey, analog.filterReleaseMs));
                analog.lfoRateHz = static_cast<float>(state.getProperty("analogLfoRate" + analogKey, analog.lfoRateHz));
